...
/-- Type quantifiers: k_ex813230_ : Nat, k_ex813230_ ∈ {16, 32, 64, 128} -/
def float_is_nan (op : (BitVec k_ex813230_)) : Bool :=
  let { exp := exp, mantissa := mantissa, sign := _ } := (float_decompose op)
  ((is_all_ones exp) && (! (is_all_zeros mantissa)))

/-- Type quantifiers: k_ex813247_ : Nat, k_ex813247_ ∈ {16, 32, 64, 128} -/
def float_is_snan (op : (BitVec k_ex813247_)) : Bool :=
...
def float_is_qnan (op : (BitVec k_ex813254_)) : Bool :=
  let { mantissa := mantissa, sign := _, exp := _ } := (float_decompose op)
